...
public class MailService {
    ...
    public void sendNoteSharedMail(Note note, User sender, User receiver, String permission) {
        ...
        String noteUrl = frontendUrl + "/notes/" + note.getId();
        sendEmail(receiver.getEmail(), subject,
                buildNoteSharedTemplate(note.getTitle(), sender.getName(), sender.getEmail(), permText, badgeStyle, noteUrl));
    }
    ...
    private String buildOtpTemplate(String otp, String actionText) {
        String body =
            "<p style=\"font-size:1.3rem;font-weight:700;color:#0f172a;margin:0 0 12px 0;\">Mã xác thực của bạn</p>" +
            "<p style=\"font-size:0.9rem;line-height:1.75;color:#475569;margin:0 0 16px 0;\">Chúng tôi nhận được yêu cầu <strong>" + actionText + "</strong> cho tài khoản của bạn. Vui lòng sử dụng mã OTP bên dưới để tiếp tục:</p>" +
    ...
    private String buildVerifyEmailTemplate(String verifyUrl) {
        String body =
            "<p style=\"font-size:1.3rem;font-weight:700;color:#0f172a;margin:0 0 12px 0;\">Chào mừng bạn đến với JotDown!</p>" +
            ...
            "<div style=\"text-align:center;margin:32px 0;\">" +
                "<a href=\"" + verifyUrl + "\" style=\"display:inline-block;padding:14px 40px;background:linear-gradient(135deg,#0b5cff 0%,#1f58ff 45%,#7c3aed 100%);color:#ffffff;text-decoration:none;border-radius:6px;font-size:0.9rem;font-weight:600;\">Xác thực Email của tôi</a>" +
            "</div>" +
    ...
    private String buildNoteSharedTemplate(String noteTitle, String senderName, String senderEmail, String permText, String badgeStyle, String noteUrl) {
        String body =
            "<p style=\"font-size:1.3rem;font-weight:700;color:#0f172a;margin:0 0 12px 0;\">Bạn có một ghi chú mới!</p>" +
            "<p style=\"font-size:0.9rem;line-height:1.75;color:#475569;margin:0 0 16px 0;\"><strong>" + senderName + "</strong> (" + senderEmail + ") vừa chia sẻ một ghi chú với bạn trên JotDown.</p>" +
            "<div style=\"background:#f8fafc;border:1px solid #e2e8f0;border-left:4px solid #0b5cff;border-radius:0 8px 8px 0;padding:20px 24px;margin:24px 0;\">" +
                "<div style=\"font-size:0.95rem;font-weight:700;color:#0f172a;margin-bottom:8px;\">" + noteTitle + "</div>" +
                "<span style=\"display:inline-block;padding:3px 12px;border-radius:20px;font-size:0.72rem;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;" + badgeStyle + "\">" + permText + "</span>" +
            "</div>" +
            "<p style=\"font-size:0.9rem;line-height:1.75;color:#475569;margin:0 0 16px 0;\">Nhấn vào nút bên dưới để xem ghi chú này:</p>" +
            "<div style=\"text-align:center;margin:32px 0;\">" +
                "<a href=\"" + noteUrl + "\" style=\"display:inline-block;padding:14px 40px;background:linear-gradient(135deg,#0b5cff 0%,#1f58ff 45%,#7c3aed 100%);color:#ffffff;text-decoration:none;border-radius:6px;font-size:0.9rem;font-weight:600;\">Mở ghi chú ngay</a>" +
            "</div>";
        return wrapInLayout(body);
    }

    /**
     * Wraps body content with shared header + footer layout from template_mail.html.

...
-- =============================================
-- 1. NHÓM QUẢN LÝ GHI CHÚ (CRUD & AUTO-SAVE)
-- =============================================

-- Lấy danh sách ghi chú cho HomePage (Hỗ trợ Grid/List View)
-- Sắp xếp: Ưu tiên Ghim (Pin) -> Thời gian Ghim -> Thời gian cập nhật mới nhất
...
-- Khởi tạo ghi chú mới (Dùng khi bấm nút "Tạo mới" trên UI)
...
-- Tính năng AUTO-SAVE: Cập nhật liên tục khi người dùng đang nhập liệu
...
-- Xóa ghi chú (Các bảng images, labels, attachments sẽ tự xóa nhờ ON DELETE CASCADE)
...
-- =============================================
-- 2. NHÓM TỔ CHỨC & TÌM KIẾM (SEARCH & PIN)
-- =============================================

-- Bật/Tắt Ghim ghi chú (Trigger sẽ tự xử lý trường pinned_at)
CREATE PROCEDURE sp_Organize_TogglePin(IN p_note_id INT)
BEGIN
    UPDATE notes SET is_pinned = NOT is_pinned WHERE id = p_note_id;
END //

-- LIVE SEARCH: Tìm kiếm với độ trễ 300ms (Tìm trong cả tiêu đề và nội dung)
...
-- Lọc ghi chú theo Nhãn (Khi chọn nhãn ở Sidebar)
...
-- =============================================
-- 3. NHÓM TÙY CHỈNH UI (COLOR & SETTINGS)
-- =============================================

-- Đổi màu sắc Card ghi chú (UI Color Picker)

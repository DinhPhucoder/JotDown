DELIMITER //
-- TRIGGER TRƯỚC KHI CẬP NHẬT (BEFORE UPDATE)
-- Tự động hóa thời gian và logic Ghim (Pin)
CREATE TRIGGER tr_Notes_BeforeUpdate
BEFORE UPDATE ON notes
FOR EACH ROW
BEGIN
    -- Luôn cập nhật mốc thời gian sửa đổi mới nhất
    SET NEW.updated_at = CURRENT_TIMESTAMP;

    -- Xử lý logic ghim ghi chú
    -- Nếu người dùng bật Ghim 
    IF OLD.is_pinned = 0 AND NEW.is_pinned = 1 THEN
        SET NEW.pinned_at = CURRENT_TIMESTAMP;
    -- Nếu người dùng bỏ Ghim 
...
-- Tự động đẩy dữ liệu vào sync_queue cho Smartphone/Tablet
...
-- Kiểm soát dữ liệu liên quan


-- Tự động cập nhật số lượng hoặc kiểm tra khi gán nhãn 
